...
`PATCHES.md`의 **현재 유지 차이**가 남도록 변경 hunk를 적용한다. 코어를 교체한 뒤
보안 패치를 별도로 기억해 재복사하는 경로는 사용하지 않는다. 옛 기능을 제거해도
그 기능과 같은 파일에 있던 파서·I/O 방어는 제거 사유가 되지 않는다. P# 역사 표는
현재 구현 목록이 아니며, 현재 유지 차이와 해당 재현 명령을 함께 검토한다.
